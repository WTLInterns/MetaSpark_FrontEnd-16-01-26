...
export default function DesignQueuePage() {
  ...
  const [designerPartsSelectedRowNos, setDesignerPartsSelectedRowNos] =
    useState([]);
  const [designerMaterialSelectedRowNos, setDesignerMaterialSelectedRowNos] =
    useState([]);
  ...
  const createOrder = () => {
    ...
    const productText = form.custom?.trim()
      ? form.custom.trim()
      : form.products || "—";
      ...
      try {
        const authDataRaw =
          typeof window !== "undefined"
            ? localStorage.getItem("swiftflow-user")
            : null;
          ...
          orders.map(async (order) => {
            ...
            try {
              const response = await fetch(
                `http://localhost:8080/status/order/${numericId}`,
                {
  ...
  const filtered = useMemo(() => {
    return orders.filter((o) => {
      const matchesQuery = `${o.id} ${o.customer}`
        .toLowerCase()
  ...
  const savePartsSelection = async () => {
    ...
    const orderId = currentPdfOrderId;
    if (!orderId) return;
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/pdf/order/${numericId}/parts-selection`,
        {
          method: "POST",
          headers: {
  ...
  const saveMaterialSelection = async () => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/pdf/order/${numericId}/material-selection`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Manage orders in the inquiry and design phase.
            </p>
          </div>
                    ...
                    <div className="flex-1 overflow-auto p-2 text-xs">
                      {isAnalyzingPdf && (
                        <div className="flex flex-col items-center justify-center h-64">
                          ...
                          <p className="text-sm text-gray-600 mb-2">
                            Analyzing PDF format...
                          </p>
                          <p className="text-xs text-gray-500">
                            Please wait while we detect the PDF type and extract
                            data.
                          </p>
                        </div>
                      )}
                      ...
                      {isRowsLoading && !isAnalyzingPdf && (
                        <div className="flex flex-col items-center justify-center h-64">
                          ...
                          <p className="text-xs text-gray-500">
                            Extracting rows and preparing checkboxes.
                          </p>
                        </div>
                      )}
                                  ...
                                  <th className="px-2 py-1 text-center">
                                    <input
                                      ...
                                      checked={
                                        pdfRows.length > 0 &&
                                        pdfRows.every((row) =>
                                          selectedSubnestRowNos.includes(
                                            row.rowNo
                                          )
                                        )
                                      }
                                ...
                                {pdfRows.map((row) => (
                                  <tr key={row.rowNo}>
                                    ...
                                    <td className="px-2 py-1 text-center">
                                      <input
                                        type="checkbox"
                                        checked={selectedSubnestRowNos.includes(
                                          row.rowNo
                                        )}
                                        onChange={(e) => {
                                          const checked = e.target.checked;
                                  ...
                                  <th className="px-2 py-1 text-center">
                                    <input
                                      ...
                                      checked={
                                        userRole === "DESIGN" &&
                                        partsRows.length > 0 &&
                                        partsRows.every((row, idx) =>
                                          designerPartsSelectedRowNos.includes(
                                            getPartsSelectionId(row, idx)
                                          )
                                        )
                                      }
                                      onChange={(e) => {
                                        ...
                                        const checked = e.target.checked;
                                        const visibleIds = partsRows.map(
                                          (row, idx) =>
                                            getPartsSelectionId(row, idx)
                                        );
                                        if (checked) {
                                ...
                                {partsRows.map((row, idx) => (
                                  <tr key={getPartsSelectionId(row, idx)}>
                                    ...
                                    <td className="px-2 py-1 text-center">
                                      <input
                                        type="checkbox"
                                        checked={designerPartsSelectedRowNos.includes(
                                          getPartsSelectionId(row, idx)
                                        )}
                                        onChange={(e) => {
                                          const checked = e.target.checked;
                                          const id = getPartsSelectionId(
                                            row,
                                            idx
                                          );
                                          setDesignerPartsSelectedRowNos(
                            ...
                            <table className="min-w-full text-xs border border-gray-200">
                                  ...
                                  <th className="px-2 py-1 text-center">
                                    <input
                                      ...
                                      checked={
                                        ...
                                        materialRows.every((_, idx) =>
                                          designerMaterialSelectedRowNos.includes(
                                            idx
                                          )
                                        )
                                      }
                                      onChange={(e) => {
                                        if (userRole !== "DESIGN") return;
                                        const checked = e.target.checked;
                                        const visibleIds = materialRows.map(
                                          (_, idx) => idx
                                        );
                              ...
                              <tbody className="text-gray-900 divide-y divide-gray-100">
                                {materialRows.map((row, idx) => (
                                  <tr
                                    key={`${row?.id ?? "mat"}-${row?.material ?? ""}-${row?.thickness ?? ""}-${row?.sizeX ?? ""}-${row?.sizeY ?? ""}-${idx}`}
                                  >
                                    <td className="px-2 py-1">{row.material}</td>
                                    <td className="px-2 py-1">{row.thickness}</td>
                                    <td className="px-2 py-1">{row.sizeX}</td>
                                    ...
                                    <td className="px-2 py-1 text-center">
                                      <input
                                        type="checkbox"
                                        checked={designerMaterialSelectedRowNos.includes(
                                          idx
                                        )}
                                        onChange={(e) => {
                                          const checked = e.target.checked;
                        ...
                        <div className="flex gap-2 mb-2">
                          {activePdfTab === "parts" && (
                            <button
                              ...
                              disabled={
                                userRole !== "DESIGN" ||
                                designerPartsSelectedRowNos.length === 0 ||
                                isGenerating
                              }
                          ...
                          {activePdfTab === "material" && (
                            <button
                              ...
                              disabled={
                                userRole !== "DESIGN" ||
                                designerMaterialSelectedRowNos.length === 0 ||
                                isGenerating
                              }
                ...
                <div className="p-4 space-y-4">
                  <div>
                    ...
                    >
                      ...
                      <option value="Tyrell Corporation">
                        Tyrell Corporation
                      </option>
